...
def processFile():
    ...
    cur_data = pd.read_csv("book2.csv")
    sentences = cur_data["cap"]
    sentences_locs = cur_data["location"]
    ...
    for i in range(0, len(unique_locs)):
        ...
        cur_sent[unique_locs[i]]=[]
        for j in range(0, len(sentences)):
            if (sentences_locs[j] == unique_locs[i]):
                cur_sent[unique_locs[i]].append(sentences[j])
    for i in range(0, len(locations)):
        locs = literal_eval(locations[i])
        for j in range(0, len(locs)):
    ...
    for i in range(0, len(unique_locs)):
        loc_sentences_cur = cur_sent[unique_locs[i]]
        loc_sentences = []
        for j in range(0, len(loc_sentences_cur)):
                ...
                if (sentArr.__len__()>10):
                    ...
                    allowed = False
                    for x in range(0, len(existingSentences)):
                        hits = 0
                        for k in range(0, len(sentArr)):
                            isNotEmpty = bool(sentArr[k])
                            lenStr = len(sentArr[k])
                            if (isNotEmpty and lenStr>2):
                                if (dictionary.check(sentArr[k])):
                                    if (re.search(sentArr[k], existingSentences[x], re.IGNORECASE)):
                                        hits = hits + 1
                        if (hits>10):
                            allowed = True
                            break
    ...
    for i in range(0, len(unique_locs)):
        ...
        loc_matches[unique_locs[i]]=loc_sentences
        '''
    with open("book4.csv", 'w', encoding='UTF8', newline='') as file:
